...
### 方法 1：Release からインストール（推奨）

1. [Releases](https://github.com/cloud-oc/PixTab/releases) ページにアクセスします。
2. 最新の `.crx` ファイルをダウンロードします。
3. ブラウザで `chrome://extensions` を開きます。
4. **デベロッパーモード** を有効にします。
5. `.crx` ファイルを拡張機能ページにドラッグ＆ドロップします。

### 方法 2：パッケージ化されていない拡張機能を読み込む
