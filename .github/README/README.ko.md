...
4. **방법 A — 패키지 파일(ZIP) 설치 (추천):**
    - `dist/` 폴더에 있는 릴리즈 패키지(예: `dist/pixtab-1.8-chrome.zip`)를 다운로드한 경우, `.zip` 파일을 확장 프로그램 관리 페이지로 드래그 앤 드롭하세요.
...
4. **방법 A — XPI 패키지 설치 (테스트용):**
    - `dist/*.xpi` 파일(예: `dist/pixtab-1.8-firefox.xpi`)을 다운로드했다면, `about:addons` 페이지에서 톱니바퀴 아이콘 ⚙️ → **파일에서 부가 기능 설치...**를 선택하거나 `.xpi` 파일을 페이지로 드래그하세요.
5. **방법 B — 임시 로드 (개발자용):**
