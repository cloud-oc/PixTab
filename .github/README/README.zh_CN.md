...
- 👤 **指定画师** — 按画师 ID 筛选，只显示特定画师的作品。
- ⭐ **关注的画师** — 展示你在 Pixiv 关注的画师的最新作品。
- 💝 **收藏作品** — 浏览你在 Pixiv 收藏的插画。
- ✨ **推荐作品** — 显示 Pixiv 为你推荐的个性化内容。
- 🔐 **登录支持** — 支持登录 Pixiv 账号，解锁个性化功能。
- 🖼️ **类型筛选** — 选择插画、漫画、动图 (Ugoira)，或屏蔽 AI 生成作品。
- 📏 **画质控制** — 设置最小宽/高，拒绝模糊图。
- 🛠️ **个性化定制** — 自由调整图片大小、对齐方式和平铺模式。
- 🌓 **主题同步** — 界面主题随系统时间自动切换深色/浅色模式。
- 🌍 **多语言支持** — 提供简体中文、繁體中文、英语、日语、韩语、俄语。
- 🌐 **反向代理** — 配置自定义代理地址，方便无法直接访问 Pixiv 的用户使用。
...
> **网络受限地区用户注意**：如果你无法直接访问 Pixiv，本扩展支持**反向代理**功能。你可以在高级设置中配置：
> 1. 打开扩展设置页面
> 2. 滚动到**高级设置**部分
> 3. 在**使用反向代理**中输入代理域名（如 `i.pixiv.re`、`i.pixiv.cat`、`i.pixiv.nl`）
> 4. 留空则不使用
> 
> 详细使用说明请参阅 [REVERSE_PROXY_GUIDE.md](../../REVERSE_PROXY_GUIDE.md)

> 如果图片仍然无法加载，请检查网络连接或尝试更换其他代理域名。注意：本扩展不提供自己的代理基础设施，而是使用第三方反向代理服务。
